...
exports.getUserActiveContracts = async (user = null, active = true, limit = 20, page = 1) => {
    ...
    const [contracts, contractsCount] = await Promise.all([
        Contract.find(query).populate({path: 'user', model: 'user', select: '_id phone balance firstName lastName'}).populate({path: 'scooter', model: 'scooter', select: '_id battery name'}).limit(limit).skip(skip).sort({createdAt: -1}).lean(),
